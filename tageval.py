...
def evaluate_taggings(goldseq_predseq_pairs, ignore_labels=False, verbose=True):
...
def evaluate_tagging_file(gold_tags_file, predicted_tags_file, verbose=True):
    ...
    return evaluate_taggings(list(zip(goldseqs, predtags)), ignore_labels=True, verbose=verbose)


def evaluate_tagging(gold_tags_file, predicted_tags):
    tokens_and_tags = read_tokens_tags_file(gold_tags_file)
    goldseqs = [tags for tokens,tags in tokens_and_tags]
    assert len(goldseqs) == len(predicted_tags)
    predtags = predicted_tags
    return evaluate_taggings(list(zip(goldseqs, predtags)), ignore_labels=True, verbose=True)
